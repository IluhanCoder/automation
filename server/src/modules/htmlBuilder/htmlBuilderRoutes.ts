...
const parseHtmlStructure = (rawHtml: string): HtmlStructureNode[] | null => {
  const html = String(rawHtml || "");
  const tokenRegex = /<\/?([a-zA-Z0-9-]+)([^>]*)>/g;
  const root: HtmlStructureNode = { tag: "__root__", className: "", children: [] };
  ...
  while ((match = tokenRegex.exec(html)) !== null) {
    ...
    const node: HtmlStructureNode = { tag, className, children: [] };
...
const areStructureNodesEqual = (a: HtmlStructureNode, b: HtmlStructureNode): boolean => {
  if (a.tag !== b.tag) return false;
  if (a.className !== b.className) return false;
  if (a.children.length !== b.children.length) return false;
  for (let i = 0; i < a.children.length; i += 1) {
    if (!areStructureNodesEqual(a.children[i], b.children[i])) return false;
  }
  return true;
};

const findFirstNodeByTag = (
  nodes: HtmlStructureNode[],
  targetTag: string
): HtmlStructureNode | null => {
...
const getTagsOutsideBody = (tree: HtmlStructureNode[]): string[] => {
  const outsideTags: string[] = [];

  const walk = (nodes: HtmlStructureNode[], insideBody: boolean) => {
    for (const node of nodes) {
...
htmlBuilderRouter.post("/api/modules/html-builder/validate", (req, res) => {
  ...
  if (!level) return res.status(404).json({ error: "Рівень не знайдено" });
  ...
  if (!htmlMatch) structureErrors.push("Відсутній кореневий тег <html> або він розташований неправильно.");
  else {
    const htmlContent = htmlMatch[0];
    // 2. head only in html
    if (!/<head[\s\S]*?<\/head>/.test(htmlContent)) structureErrors.push("<head> має бути всередині <html>.");
    // 3. body only in html
    if (!/<body[\s\S]*?<\/body>/.test(htmlContent)) structureErrors.push("<body> має бути всередині <html>.");
  ...
  if (!submittedTree) {
    ...
    if (tagsOutsideBody.length > 0) {
      structureErrors.push(
        `Теги мають бути всередині <body>: ${tagsOutsideBody.join(", ")}.`
      );
    }
  }
  ...
  if (isCorrect) {
    ...
    if (missingTags.length > 0) feedback += `Не вистачає тегів: ${missingTags.join(", ")}. `;
    if (structureErrors.length > 0) feedback += `Помилки структури: ${structureErrors.join(" ")} `;
    if (!htmlMatchesTarget) feedback += "Візуальна структура (порядок/вкладеність/класи) не відповідає зразку.";
  }
...
htmlBuilderRouter.post("/api/modules/html-builder/answer", async (req, res) => {
  ...
  try {
    ...
    const htmlMatch = html.match(/<html[\s\S]*?<\/html>/);
    if (!htmlMatch) structureErrors.push("Відсутній кореневий тег <html>.");
    else {
      const htmlContent = htmlMatch[0];
      if (!/<head[\s\S]*?<\/head>/.test(htmlContent)) structureErrors.push("<head> має бути всередині <html>.");
      if (!/<body[\s\S]*?<\/body>/.test(htmlContent)) structureErrors.push("<body> має бути всередині <html>.");
    ...
    if (isCorrect) {
      ...
      if (missingTags.length > 0) feedback += `Не вистачає: ${missingTags.join(", ")}. `;
      if (structureErrors.length > 0) feedback += `${structureErrors.join(" ")} `;
      if (!htmlMatchesTarget) feedback += "Візуальна структура (порядок/вкладеність/класи) не відповідає зразку.";
    }
