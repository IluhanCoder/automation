...
gameRouter.post("/api/games", async (req, res) => {
  try {
    ...
    const sanitizedLevels: LevelConfig[] = levels.map((level: LevelConfig) => ({
      id: String(level.id),
      points: Number(level.points),
    }));

    const hasInvalidLevel = sanitizedLevels.some(
      (level) => !level.id || !Number.isFinite(level.points) || level.points < 0
    );
